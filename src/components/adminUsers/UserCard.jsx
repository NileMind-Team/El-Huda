  ...
  return (
    <motion.div
      ...
      className={`bg-white backdrop-blur-sm rounded-xl sm:rounded-2xl p-3 sm:p-4 md:p-5 lg:p-6 border-2 transition-all duration-300 ${
        isCurrentUser(user)
          ? "border-[#F18E02] shadow-lg hover:shadow-xl"
          : user.isActive === false
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 sm:gap-4">
        <div className="flex items-start gap-3 sm:gap-4 flex-1 min-w-0">
          <div className="flex-shrink-0 relative">
              ...
              <div
                className={`w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 rounded-full flex items-center justify-center font-semibold text-base sm:text-lg md:text-xl border-2 ${
                  user.isActive === false
                    ...
                    : "bg-[#F18E02] text-white border-[#F18E02]"
                }`}
              >
            ...
            {isCurrentUser(user) && (
              <div className="absolute -top-1 -right-1 bg-[#F18E02] text-white rounded-full p-1 border-2 border-white">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-col xs:flex-row xs:items-center gap-2 xs:gap-3 mb-2 sm:mb-3">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {isCurrentUser(user) && (
                  <span className="bg-[#F18E02] text-white px-2 py-1 rounded-full text-xs font-semibold whitespace-nowrap border border-[#F18E02]">
                    المستخدم الحالي
                  </span>
                )}
                {getStatusBadge(user)}
              ...
              <div className="flex items-center gap-2">
                <FaEnvelope
                  className={`flex-shrink-0 text-xs sm:text-sm ${
                    user.isActive === false ? "text-gray-400" : "text-[#F18E02]"
                  }`}
                />
